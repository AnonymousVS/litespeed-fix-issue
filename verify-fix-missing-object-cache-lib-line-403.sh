...
LOG_FILE="/var/log/lscwp-verify-objcache-lib.log"
LOCK_FILE="${LOG_FILE}.lock"
RESULT_DIR="/tmp/lscwp-verify-lib-$$"
RAM_PER_JOB_MB=200
WP_TIMEOUT=30
...
log "======================================"
log " VERIFY object-cache.php /lib/"
...
verify_site() {
    ...
    if [ -f "$LIB_FILE" ]; then
        ...
        _log "❌ MISSING object-cache.php: $SITE"
        ( flock 200; echo "$dir" >> "$RESULT_DIR/failed_sites.txt" ) 200>"$LOCK_FILE"
        touch "${RESULT_DIR}/fail_${UNIQUE}" 2>/dev/null
    fi
}
...
if [ "$FAIL" -gt 0 ] && [ -f "$RESULT_DIR/failed_sites.txt" ]; then
    ...
    log "======================================"
    log " รัน Fix อีกครั้ง:"
    log " curl -s https://raw.githubusercontent.com/ufavision/server-scripts/main/fix-missing-object-cache-lib-line-403.sh | bash"
    log "======================================"
fi
```
...
## อัปโหลดขึ้น GitHub ชื่อนี้
```
verify-fix-missing-object-cache-lib-line-403.sh
